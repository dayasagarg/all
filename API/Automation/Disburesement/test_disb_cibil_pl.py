...
class TestLoanDisbCibil:
    ...
    def test_disb_cibil_new_user(self, url_dis_cibil):
        ...
        for r in disAPIData:
            if r["Completed loans"] <= 3:
                ...
                if (r["Cibil Score"] < 700) and (r["Pl Score"] < 700):
                    # print("lid::",r["Loan ID"])
                    cibil_pl_less_than_700_lid.append(r["Loan ID"])

                elif (700 >= r["Cibil Score"] <= 749) and 700 >= r["Pl Score"] <= 749:
                ...
                elif (r["Approved Salary"] <= 50000) and (750 >= r["Cibil Score"] <= 799) and (750 >= r["Pl Score"] <= 799):
                ...
                elif r["Approved Salary"] >= 100000 and (750 >= r["Cibil Score"] <= 799) and (750 >= r["Pl Score"] <= 799):
                    # if (750 >= r["Cibil Score"] <= 799) or (750 >= r["Pl Score"] <= 799):
                ...
                elif r["Cibil Score"] >= 800 and r["Pl Score"] >= 800:
                    ...
                    #
                    # print("lid::",r["Loan ID"])
                    # print("apprSalary_loan_amt_cal::",apprSalary_loan_amt_cal)
                    # print("apprSalary_loan_amt_cal_500_round::",apprSalary_loan_amt_cal_500_round)
                    # print("apprAmt::", apprAmt)
    ...
    def test_cs_pl_less_than_700(self):
        if cibil_pl_less_than_700_lid_count > 0:
            print(f" Error :: loans are not rejected if cibil_and_pl_less_than_700:: {cibil_pl_less_than_700_lid}")
            assert False
        else:
            print("*** loans are rejected if cibil_and_pl_less_than_700 ***")
    ...
    def test_disb_cibil_repeat_user(self, url_dis_cibil):
        ...
        for s in disAPIData:
            if s["Completed loans"] > 3:
                ...
                elif s["Approved Salary"] <= 50000 and (750 >= s["Cibil Score"] <= 799):
                    # if (750 >= r["Cibil Score"] <= 799) or (750 >= r["Pl Score"] <= 799):
                ...
                elif s["Approved Salary"] >= 100000 and (750 >= s["Cibil Score"] <= 799):
                    # if (750 >= r["Cibil Score"] <= 799) or (750 >= r["Pl Score"] <= 799):
    ...
    def test_cs_pl_less_than_700_r(self):
        if cibil_pl_less_than_700_lid_count_r > 0:
            print(f" Error :: loans are not rejected if cibil_and_pl_less_than_700 ::Repeat User:: {cibil_pl_less_than_700_lid_r}")
            assert False
        else:
            print("*** loans are rejected if cibil_and_pl_less_than_700 ::Repeat User:: ***")
