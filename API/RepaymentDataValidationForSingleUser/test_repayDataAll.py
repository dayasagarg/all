...
class TestRepayment:
    def test_getRepayment(self):
        ...
        response = requests.get(
            "https://lendittfinserve.com/prod/admin/transaction/allRepaidLoans?start_date=2023-10-07T10:00:00.000Z&end_date=2023-10-07T10:00:00.000Z&page=1&pagesize=10&getTotal=true",
        ...
        emiData = response.json()["data"]["rows"]
        # print(emiData)
        ...
        for i in tranData:
            if i["status"] == "COMPLETED":
                if "paidAmount" in i:
                    tPaidAmount.append(i['paidAmount'])
                    # print(i['EMI date'])

                if "principalAmount" in i:
                    tPrincipalAmount.append(i['principalAmount'])

                if "principalDifference" in i:
                    tPrincipalDifference.append(i['principalDifference'])

                if "interestAmount" in i:
                    tInterestAmount.append(i['interestAmount'])

                if "interestDifference" in i:
                    tInterestDifference.append(i['interestDifference'])

                if "penaltyAmount" in i:
                    tPenaltyAmount.append(i['penaltyAmount'])

                if "penaltyDifference" in i:
                    tPenaltyDifference.append(i['penaltyDifference'])
