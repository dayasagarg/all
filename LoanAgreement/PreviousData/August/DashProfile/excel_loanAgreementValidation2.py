...
class TestDashRepo:
    ...
    def setup_teardown(self):
        global driver,path,row,loanID,loanPdf
        ...
        time.sleep(2)
        path = r"C:\Users\lendi\OneDrive\Desktop\Users\UserData.xlsx"
        row = xlutls.getRowCount(path, "Sheet2")
        time.sleep(2)
        for r in range(2,row+1):
            email = xlutls.readData(path, "Sheet1", r, 1)
            password = xlutls.readData(path, "Sheet1", r, 2)
            otp = xlutls.readData(path, "Sheet1", r, 3)
            loanID = xlutls.readData(path, "Sheet1", r, 4)
            loanPdf = xlutls.readData(path, "Sheet1", r, 5)
            # time.sleep(1)
    ...
    def test_keyFactStatement(self, setup_teardown):
        time.sleep(1)
        driver.find_element(By.ID, "mainSearch").send_keys(loanID)  #loan id
        time.sleep(1)
        ...
        # # Date of Signing
        loanAppDate = driver.find_element(By.XPATH,"//tr[contains(@class,'mat-row cdk-row loan-history-row bg-greywhite ng-star-inserted')]//td[5]").text
        time.sleep(1)
        ...
        loanInterestPerAnnum = str(loanIntPerAnnum) + "00%"
        if loanInterestPerAnnum in firstPage:
            print(f" *** 'loanInterestPerAnnum' :'{loanInterestPerAnnum}' is matched with first Page of pdf *** ")
        else:
            print(f"Error :: 'loanInterestPerAnnum' :'{loanInterestPerAnnum}' is not matched with first Page of pdf ")

        assert loanInterestPerAnnum in firstPage, "loanInterestPerAnnum is matched with first Page of pdf"
        ...
        if sgst in firstPage:
            print(f" *** 'sgst' :'{sgst}' is matched and within first Page of pdf *** ")
        else:
            print(f"Error :: 'sgst' :'{sgst}' is not matched and not within first Page of pdf ")

        assert sgst in firstPage, "is matched and within first Page of pdf"
        ...
        if cgst in firstPage:
            print(f" *** 'cgst' :'{cgst}' is matched and within first Page of pdf *** ")
        else:
            print(f"Error :: 'cgst' :'{cgst}' is not matched and not within first Page of pdf ")

        assert cgst in firstPage, "is matched and within first Page of pdf"
        ...
        # PAN number
        pan = driver.find_element(By.XPATH,"//div[normalize-space()='BILPA8300D']").text
        ...
        #per annum
        if loanInterestPerAnnum in sixthPage:
            print(f" *** 'loanInterestPerAnnum' :'{loanInterestPerAnnum}' is matched with sixth Page of pdf *** ")
        else:
            print(f"Error :: 'loanInterestPerAnnum' :'{loanInterestPerAnnum}' is not matched with sixth Page of pdf ")

        assert loanInterestPerAnnum in sixthPage, "loanInterestPerAnnum is matched with sixth Page of pdf"
